...
class DayNightSystem extends AbstractSystem
{
    ...
    public function render(World $world): void
    {
        ...
        // Apply fog overrides from game-side weather system (e.g., dense fog, sandstorm)
        if ($cycle->fogNearOverride >= 0.0) {
            $fogNear = $cycle->fogNearOverride;
        }
        if ($cycle->fogFarOverride >= 0.0) {
            $fogFar = $cycle->fogFarOverride;
        }
        ...
        // --- Weather surface uniforms (set by game-side AtmosphericEnvironmentalSystem via DayNightCycle) ---
        $this->commandList->add(new SetWeatherUniforms(
            rainIntensity: $cycle->weatherRainIntensity,
            snowCoverage: $cycle->weatherSnowCoverage,
            temperature: $cycle->weatherTemperature,
            dewWetness: $cycle->weatherDewWetness,
            stormIntensity: $cycle->weatherStormIntensity,
        ));
