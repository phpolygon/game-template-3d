...
class Window
{
    ...
    /**
     * Maximize the window to fill the screen and return the resulting dimensions.
     * Call after initialize() — requires a valid GL context.
     * Uses glfwMaximizeWindow (no glfwGetVideoMode which corrupts PHP memory via FFI).
     *
     * @return array{width: int, height: int} The resulting window size
     */
    public function detectAndResize(): array
    {
        ...
        $newW = 0; $newH = 0;
        glfwGetWindowSize($this->handle, $newW, $newH);
        $this->width = is_int($newW) && $newW > 0 ? $newW : 1280;
        $this->height = is_int($newH) && $newH > 0 ? $newH : 720;

        // Re-read framebuffer
        $fbW = 0; $fbH = 0;
        glfwGetFramebufferSize($this->handle, $fbW, $fbH);
        $this->framebufferWidth = is_int($fbW) ? $fbW : $this->width;
        $this->framebufferHeight = is_int($fbH) ? $fbH : $this->height;

        // Re-read content scale
        $csX = 1.0; $csY = 1.0;
        glfwGetWindowContentScale($this->handle, $csX, $csY);
